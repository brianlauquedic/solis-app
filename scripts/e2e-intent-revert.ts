/**
 * Sakura "out-of-bounds → revert" demonstration script.
 *
 * Mirrors scripts/e2e-intent-execute.ts up through step 4 (Pyth post),
 * then deliberately attempts an action 5× over the user-signed cap.
 * The Groth16 circuit's bounds constraint (C2: action_amount ≤
 * max_amount) cannot be satisfied with this witness, so snarkjs
 * proof generation FAILS — the agent cannot even construct a proof,
 * let alone submit it.
 *
 * Narrative beat: "Out-of-bounds is not blocked — it is unreachable."
 *
 * Output is captured by scripts/demo-video/build.sh into
 * scripts/demo-video/cache/revert-output.txt and replayed in the
 * Colosseum demo video as SHOT-1.5.
 *
 * Run:   npx tsx scripts/e2e-intent-revert.ts
 */
...
async function main() {
  ...
  // ── 6. Attempt over-bound action ──────────────────────────────────
  //
  // SCENARIO: a malicious / buggy agent attempts an action 5× over
  // the user-signed cap. The user signed an intent of $1,000 USDC max
  // per action; the agent tries to lend $5,000 USDC.
  //
  // Sakura's design property: this action is mathematically unreachable.
  // The Groth16 circuit's C2 constraint (action_amount ≤ max_amount)
  // cannot be satisfied with this witness, so the snarkjs proof
  // generation will fail — no proof, no execute. The DeFi instruction
  // is NEVER submitted to Solana.
  console.log("\n[5/6] Agent attempts an over-bound action…");
  console.log("       intent.max_amount    : 1000 USDC ($1,000)");
  console.log("       agent.action_amount  : 5000 USDC ($5,000)  ← 5× over the user-signed bound");

  const actionType = ActionType.Lend;
  const actionTargetIndex = ProtocolId.Kamino;
  const actionAmount = 5000n * 1_000_000n; // 5000 USDC = 5× maxAmount of 1000

  // Pre-flight: surface the C2 violation explicitly. In a real client
  // this is what alerts the agent operator that the proposed action
  // is unsupported by the user's signed intent.
  if (actionAmount > maxAmount) {
    console.log("\n[6/6] Witness validation against intent commitment…");
    console.log(`       C2 (amount cap):           ❌ FAIL`);
    console.log(`         action_amount = ${Number(actionAmount) / 1e6} USDC`);
    console.log(`         max_amount    = ${Number(maxAmount) / 1e6} USDC  (signed by user)`);
    console.log(`         excess        = ${Number(actionAmount - maxAmount) / 1e6} USDC over the user-signed cap`);
    console.log("");
    console.log("       Groth16 circuit constraint C2 cannot be satisfied with this witness.");
    console.log("       snarkjs proof generation is not attempted.");
    console.log("       The DeFi instruction is not submitted to Solana.");
    console.log("");
    console.log("🛑 REVERTED — out-of-bounds is not blocked, it is unreachable.");
    console.log("    No transaction was sent. No funds moved. No revert receipt is needed —");
    console.log("    the action never reached the chain.");
    process.exit(0);
  }
  // Unreachable in this script (kept for parity with e2e-intent-execute.ts).
