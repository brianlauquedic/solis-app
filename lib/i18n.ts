...
export const translations = {
  ...
  // ── Stats ────────────────────────────────────────────────────────
  statLabel1: { zh: "家 Solana 錢包今年準備出代理版，都需要這層", en: "Solana wallets shipping agent modes this year — all need this layer", ja: "社の Solana ウォレットが今年エージェント版を出す予定 — すべてにこの層が必要" },
  statLabel2: { zh: "k CU — 代理每次動手的鏈上成本", en: "k CU — on-chain cost per agent action", ja: "k CU — エージェントアクション毎のオンチェーンコスト" },
  statLabel3: { zh: "B+ Solana DeFi TVL 可經代理安全調度", en: "B+ Solana DeFi TVL safely addressable via bounded agents", ja: "B+ Solana DeFi TVL — 境界付エージェントで安全にアクセス可能" },
  statLabel4: { zh: "% 的代理動作必須通過數學閘門，無一例外", en: "% of agent actions pass the math gate — no exceptions", ja: "% のエージェントアクションが数学ゲートを通過 — 例外なし" },
