...
export const translations = {
  ...
  heroSubtitle: {
    zh: "Solana 原生的代理執行邊界層。基於 alt_bn128 配對系統呼叫，116,000 計算單元鏈上驗證，單次成本 $0.0001，運營方無從放寬。",
    en: "Solana's native execution-bounds layer for AI agents. Built on the alt_bn128 pairing syscall. 116,000 CU on-chain verification, $0.0001 per call, zero operator override.",
    ja: "AI エージェントのための Solana ネイティブ実行境界層。alt_bn128 ペアリングシステムコールを基盤とし、116,000 CU のオンチェーン検証、1 回 $0.0001、運用者による緩和は不可能。",
  },
  ...
  diffDesc: {
    zh: "每一次代理動作，閘門都要做四項檢查：動作是否滿足已簽意圖、Pyth 價格是否與鏈上當前狀態一致、slot 是否在 150 塊新鮮度窗口內、action nonce 是否重放。Phantom、Backpack、Abstract、Infinex 若各自建造，成本以工程師季度計，且每次協議升級都要重做。\n\nSakura 把這一層做成公用。鏈上閘門單次驗證約 116,000 計算單元，成本 $0.0001。前 $10M 整合量對首批錢包免費，此後 0.1% 的名義金額由整合方自動路由至協議金庫，其中 15% 進入 3/5 多簽治理的平台財庫——參數在程式級天花板（2% 與 100%）內可調，超越需重新部署。\n\n最貼近的歷史類比是 HTTPS 憑證發行：全球每條加密 web 連線的閘口，三十年來無代幣、按次收費、由獨立機構分散運營。Sakura 走同一範式。",
    en: "Every gate invocation performs four checks: the action satisfies the signed intent, the Pyth price matches the on-chain state, the slot falls within a 150-block freshness window, and the action nonce has not been replayed. If Phantom, Backpack, Abstract, and Infinex each build this independently, the all-in cost is measured in engineer-quarters and recurs with every protocol upgrade.\n\nSakura builds the layer once, for everyone. On-chain verification costs roughly 116,000 compute units — about $0.0001 per call. First-wave integrators pay zero on the first $10M of notional routed through the gate; subsequently, 0.1% of notional is routed automatically to the protocol fee vault, 15% of which accrues to a 3-of-5 multisig-governed treasury. Parameters are tunable within program-level ceilings (2% and 100%). Exceeding them requires a full redeployment.\n\nThe closest historical analogue is HTTPS certificate issuance: the gate every encrypted connection on the web passes through, operated for three decades on a token-free, cents-per-issuance model by independent authorities. Sakura is built on the same template.",
    ja: "ゲートの各呼び出しは 4 つを検証する——アクションが署名済み意図を満たすこと、Pyth 価格がオンチェーンの状態と一致すること、スロットが 150 ブロックの新鮮度ウィンドウ内にあること、アクションノンスが再生されていないこと。Phantom、Backpack、Abstract、Infinex が各自これを構築する場合、総コストはエンジニア四半期単位で発生し、プロトコルアップグレード毎に再発生する。\n\nSakura はこの層を一度構築し、共有する。オンチェーン検証は約 116,000 計算ユニット、1 回 $0.0001。第一波統合者は最初の $10M の名目額まで無料、以降は名目額の 0.1% がプロトコル手数料金庫に自動ルーティングされ、15% が 3/5 マルチシグ統治のトレジャリーへ入る。両パラメータはプログラムレベルの上限（2% と 100%）内で調整可能、超越には再デプロイを要する。\n\n最も近い歴史的類似物は HTTPS 証明書発行——世界のあらゆる暗号化接続が通るゲートであり、30 年間、独立機関によってトークン無し・1 回数セントで運営されてきた。Sakura は同じテンプレートに基づいて設計されている。",
  },
  ...
  sakuraCharacterDesc: {
    zh: "每一個 AI 代理錢包最終都要回答同一個問題：當代理替你動手，什麼在阻止它做你沒授權的事？\n\n過去三十年裡，這個問題的答案從來都是一套政策。網銀在 2000 年代給過一次答案；託管型交易所在 2014、2019、2022、2024 年，用用戶的錢重複給過四次。每一次都以同樣的方式收場。\n\nSakura 把這個問題交給數學。用戶以自然語言寫下一條指令——「代理可在 Kamino 借貸，單次不超過 $1,000，為期一週」——經 Poseidon 壓縮為 32 位元組承諾，寫入用戶在 Solana 鏈上的 Intent PDA。此後代理每一次動作，都必須附一份 Groth16 零知識證明，論證該動作落在承諾範圍內。Solana 協議 1.17 合入的 alt_bn128 配對系統呼叫，在 116,000 計算單元內完成驗證，單次約 $0.0001。證明通過，DeFi 指令原子執行；證明失敗，整筆交易回滾。\n\n運營方無法放寬規則，協議維護者無法放寬規則，未來的治理機構亦無法放寬規則。這不是承諾不會越界——這是在數學上無法越界。",
    en: "Every AI-agent wallet will, eventually, answer the same question: when the agent acts on your behalf, what stops it from doing something you never authorized?\n\nFor thirty years, the answer has been a policy. Online banking settled it in the 2000s. Custodial exchanges resettled it — at significant user cost — in 2014, 2019, 2022, and 2024. Each resolution ended the same way.\n\nSakura hands the question to mathematics. A user signs, once, a natural-language instruction — \"the agent may lend up to $1,000 into Kamino for one week\" — compressed via Poseidon into a 32-byte commitment written to the user's Intent PDA on Solana. Every subsequent action the agent attempts must carry a Groth16 zero-knowledge proof that the action falls inside the commitment. Solana's alt_bn128 pairing syscall, merged in protocol 1.17, verifies the proof in 116,000 compute units — roughly one hundredth of a cent per call. Proof passes, the DeFi instruction lands atomically. Proof fails, the entire transaction reverts.\n\nThe agent's operator cannot widen the rule. The protocol's maintainers cannot widen the rule. A future governance body cannot widen the rule. This is not a commitment to behave. It is a structural impossibility.",
    ja: "すべての AI エージェントウォレットは、最終的に同じ問いに答えることになる——エージェントがあなたの代わりに動くとき、あなたが認可していない行為をさせないのは、何か。\n\n過去 30 年、この問いへの答えは常にポリシーだった。オンラインバンキングが 2000 年代に一度決着させ、カストディアル取引所が 2014、2019、2022、2024 年に、ユーザーに重大な代償を払わせながら再決着させた。どの決着も、同じ終わり方をした。\n\nSakura はこの問いを数学に委ねる。ユーザーは自然言語で一度だけ指令に署名する——「エージェントは Kamino に、1 回最大 $1,000、1 週間だけ貸せる」——この指令は Poseidon で 32 バイトのコミットメントにハッシュされ、Solana 上のユーザーの Intent PDA に書き込まれる。以降エージェントが提案するすべてのアクションは、そのアクションがコミットメント内に収まることを示す Groth16 ゼロ知識証明を伴わなければならない。Solana プロトコル 1.17 で合流した alt_bn128 ペアリングシステムコールが、116,000 計算ユニットで——1 回あたり約 1 セントの 100 分の 1 で——この証明を検証する。証明が通れば DeFi 命令がアトミックに着地し、失敗すればトランザクション全体がリバートする。\n\nエージェントの運用者は、規則を広げられない。プロトコルの保守者も、規則を広げられない。将来のガバナンス機構も、規則を広げられない。これは越境しないという約束ではない。構造的に不可能なのである。",
  },
  sakuraMission: {
  ...
  ctaFreeDesc: {
    zh: "連接 Phantom 或 OKX，以自然語言寫下代理的權限邊界：「代理可在 Kamino 借貸，單次最多 $500 USDC，為期一週」。Sakura 編碼為 32 位元組鏈上承諾，此後每次代理動作都須通過 Groth16 配對驗證。鏈上成本 $0.01 單次，整合無需許可，源碼 MIT 授權。",
    en: "Connect Phantom or OKX and state the agent's permissions in one sentence: \"the agent may lend up to $500 USDC into Kamino for one week.\" Sakura encodes it as a 32-byte on-chain commitment, and every agent action thereafter must pass Groth16 pairing verification. $0.01 on-chain per call. Permissionless integration. MIT-licensed source.",
    ja: "Phantom または OKX を接続し、エージェントの権限を 1 文で記述する——「エージェントは Kamino に、1 回最大 $500 USDC、1 週間だけ貸せる」。Sakura はこれを 32 バイトのオンチェーン・コミットメントに符号化し、以降のエージェントアクションは毎回 Groth16 ペアリング検証を通過する必要がある。1 回 $0.01、許可不要の統合、MIT ライセンスのソース。",
  },
