...
export const translations = {
  ...
  // ── Value-capture pointer (ceilingBody footer line) ──────────────
  // Renders as the final line of the 算 · DAY-1 SOM landing section.
  // Surfaces docs/VALUE_CAPTURE.md (which otherwise only appears as a
  // tiny badge in the README) so the "how do you make money" question
  // has a visible answer on the primary landing copy, not only in docs.
  valueCaptureLine: {
    zh: "Sakura 如何捕獲價值：5 項定價操作、3 項已在 devnet 上運行（sign_intent 0.1%、執行 $0.01/action、MCP $1/call），~85-99% 毛利率。無代幣，靠費用流供養。完整拆解見 `docs/VALUE_CAPTURE.md`。",
    en: "How Sakura captures value: 5 priced operations, 3 live on devnet (sign_intent 0.1%, execute $0.01/action, MCP $1/call), ~85-99% gross margin. No token — the primitive sustains on fee flow. Full breakdown in `docs/VALUE_CAPTURE.md`.",
    ja: "Sakura の価値捕獲：5 つの有償操作、うち 3 つは devnet で稼働中（sign_intent 0.1%、実行 $0.01/action、MCP $1/call）、粗利率 ~85-99%。トークンなし、手数料フローで運営。完全な内訳は `docs/VALUE_CAPTURE.md` に。",
  },
} as const;
