    ...
    sections: [
      {
        ...
        steps: [
          { step: "壱", title: "描述寫得太抽象", desc: "糟糕例子：「代理做一些有收益的事」。AI 對「有收益」有自己的理解——你以為它會 Lend，它可能認為「Borrow 然後 Swap 再 Lend」也算。好的例子：「代理可以把最多 \$500 USDC 存入 Kamino 或 MarginFi，每週結算一次」。具體，可查證，界線清楚。" },
          { step: "弐", title: "勾選了「反正也沒差」的協議", desc: "為了省事把 5 個協議（Kamino、MarginFi、Jupiter、Marinade、Jito）都勾上。問題是：每個協議的失敗模式不同——MarginFi 支援借貸（清算風險）、Jito / Marinade 是流動質押（LST 脫鉤風險）、Jupiter 是聚合 Swap（MEV / 滑點）、Kamino 是借貸市場。一個沒做功課的勾選 = 同意代理在你不理解的風險裡做決定。原則：只勾你親自用過、知道風險特徵的協議。不熟的不勾，損失有限。" },
      ...
      },
      {
        ...
        steps: [
          ...
          { step: "弐", title: "根據策略勾最少的協議 + 動作", desc: "保守：Kamino 或 MarginFi × Lend。中等：Kamino / MarginFi / Jupiter × Lend + Swap。激進：加上 MarginFi × Borrow（產生利息債務 + 清算風險）。原則是「勾到足夠跑策略即可，多一個都不要」。每多勾一個協議 / 動作，就是多給 AI 一扇門。" },
          { step: "参", title: "寫自然語言描述——具體、可查證", desc: "不要寫「代理幫我賺錢」這種話。寫「代理可以在 Kamino 或 MarginFi 的 USDC 市場存款；不能提領或 Swap；每週檢視一次」。你之後回頭看 ActionHistory，這段話是你對照代理行為的唯一基準——寫具體，以後才有得核對。" },
      ...
      },
      {
        ...
        risks: [
          ...
          { level: "✅ Sakura 替你保證", color: "#5A7A4A", desc: "用過期或假的預言機報價 → 整筆交易 revert。Sakura 檢查 Pyth 的 feed_id、slot、150 塊新鮮度窗口。過期的報價不能被用來構造通過的證明。" },
          ...
          { level: "❌ Sakura 不替你保證", color: "#C9312A", desc: "Pyth 報價本身被操縱時（市場上真價是 \$100，Pyth 說 \$200），Sakura 只檢查「來自 Pyth」不檢查「Pyth 說的對不對」。這是預言機層面的信任，不是 Sakura 能承擔的。" },
        ],
      },
    ],
    ...
    sections: [
      {
        ...
        steps: [
          { step: "I", title: "Writing the description too abstractly", desc: "Bad example: \"Agent does something profitable.\" The AI has its own interpretation of \"profitable\" — you meant Lend, but it may read \"Borrow then Swap then Lend\" as also profitable. Good example: \"Agent may lend up to \$500 USDC into Kamino or MarginFi, settled weekly.\" Specific, verifiable, clearly bounded." },
          { step: "II", title: "Ticking protocols \"just in case\"", desc: "For convenience, all 5 protocols (Kamino, MarginFi, Jupiter, Marinade, Jito) get checked. Problem: every protocol has a different failure mode — MarginFi supports borrowing (liquidation risk), Jito / Marinade are liquid staking (LST-depeg risk), Jupiter is a swap aggregator (MEV / slippage), Kamino is a lending market. An uninformed tick equals consent to the agent making decisions inside risks you do not understand. Principle: tick only protocols you have personally used and whose risk profile you know. Skip what you don't recognize — the loss upside is bounded." },
      ...
      },
      {
        ...
        steps: [
          ...
          { step: "II", title: "Tick the minimum protocols + actions needed", desc: "Conservative: Kamino or MarginFi × Lend. Medium: Kamino / MarginFi / Jupiter × Lend + Swap. Aggressive: add MarginFi × Borrow (interest debt + liquidation risk). Principle is \"tick enough to run the strategy, not one more.\" Every extra protocol or action you tick is another door you hand to the AI." },
          { step: "III", title: "Write the natural-language description — specific, auditable", desc: "Do not write \"agent makes me money.\" Write: \"agent may deposit into Kamino or MarginFi USDC markets; may not withdraw or swap; reviews weekly.\" When you later come back to ActionHistory, this sentence is your sole reference for whether the agent behaved — so write something you can check against later." },
      ...
      },
      {
        ...
        risks: [
          ...
          { level: "✅ Sakura GUARANTEES", color: "#5A7A4A", desc: "Stale or spoofed oracle prices → transaction reverts. Sakura verifies Pyth feed_id, slot, and a 150-block freshness window. A stale price cannot be used to construct a passing proof." },
          ...
          { level: "❌ Sakura DOES NOT guarantee", color: "#C9312A", desc: "If Pyth itself is manipulated (real market price \$100, Pyth says \$200), Sakura only verifies \"price came from Pyth\" — not \"Pyth is telling the truth.\" Oracle-layer trust is not something Sakura can carry on your behalf." },
        ],
      },
    ],
    ...
    sections: [
      {
        ...
        steps: [
          { step: "壱", title: "説明が抽象的すぎる", desc: "悪い例：「エージェントは利益の出ることをする」。AI は「利益」を独自に解釈する——あなたは Lend のつもりでも、AI は「Borrow → Swap → Lend」も利益だと解釈するかもしれぬ。良い例：「エージェントは最大 \$500 USDC を Kamino または MarginFi に貸せる、週次精算」。具体的で、検証可能で、境界が明確。" },
          { step: "弐", title: "「ついでに」とプロトコルを選びすぎる", desc: "手間を省くために 5 つのプロトコル（Kamino、MarginFi、Jupiter、Marinade、Jito）すべてにチェックを入れる。問題：各プロトコルは、失敗モードが互いに異なる——MarginFi は借貸を扱う（清算リスク）、Jito / Marinade は流動性ステーキング（LST デペッグリスク）、Jupiter は Swap アグリゲータ（MEV・スリッページ）、Kamino は貸付市場。不勉強な選択は、エージェントが、あなたの理解せぬリスクの中で判断を下すことへの同意である。原則：自分で使ったことがあり、リスクプロファイルを知っているプロトコルだけを選ぶ。不慣れなものは外せ——損失の上限が有限のままである。" },
      ...
      },
      {
        ...
        steps: [
          ...
          { step: "弐", title: "戦略に基づき、必要最小限のプロトコル + 動作を選ぶ", desc: "保守：Kamino または MarginFi × Lend。中程度：Kamino / MarginFi / Jupiter × Lend + Swap。攻撃的：MarginFi × Borrow を加える（利子債務 + 清算リスクを伴う）。原則は「戦略を実行するのに足るだけ選び、余分に 1 つも加えぬ」。余分に選ぶ 1 つは、AI に渡す扉が 1 つ増えるに等しい。" },
          { step: "参", title: "自然言語で説明を書く——具体的に、検証可能に", desc: "「エージェント、儲けて」のような文は書くな。代わりに：「エージェントは Kamino または MarginFi の USDC 市場に預金できる。引き出しも Swap も不可。週次で確認する」。後日 ActionHistory に戻る際、この一文がエージェントの挙動を照合する唯一の基準となる——後で照合できるよう、具体的に書け。" },
      ...
      },
      {
        ...
        risks: [
          ...
          { level: "✅ Sakura の保証", color: "#5A7A4A", desc: "陳腐化したり偽造されたオラクル価格 → トランザクションはリバートする。Sakura は Pyth の feed_id、slot、150 ブロック新鮮度ウィンドウを検証する。陳腐化した価格で通過する証明は作れぬ。" },
          ...
          { level: "❌ Sakura の保証外", color: "#C9312A", desc: "Pyth 自身が操作された場合（実勢 \$100 なのに Pyth が \$200 と言う）、Sakura は「価格が Pyth から来たこと」のみ検証し、「Pyth が真実を語っているか」は検証せぬ。オラクル層の信頼は、Sakura が肩代わりできるものではない。" },
        ],
      },
    ],
