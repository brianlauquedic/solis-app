...
const CONTENT: Record<Lang, {
  ...
  zh: {
    ...
    sections: [
      {
        ...
        steps: [
          ...
          { step: "弐", title: "勾選了「反正也沒差」的協議", desc: "為了省事把 8 個協議全勾上。問題是：Drift 做的是衍生品，和 Kamino 的 USDC 借貸不在同一個風險量級。一個沒做功課的勾選 = 同意代理去 Drift 開槓桿。原則：只勾你親自用過、知道風險特徵的協議。不熟的不勾，損失有限。" },
      ...
      },
      {
        ...
        steps: [
          ...
          { step: "弐", title: "根據策略勾最少的協議 + 動作", desc: "保守：Kamino 或 MarginFi × Lend。中等：Kamino / MarginFi / Jupiter × Lend + Swap。激進：再加 Drift × Borrow。原則是「勾到足夠跑策略即可，多一個都不要」。每多勾一個協議 / 動作，就是多給 AI 一扇門。" },
    ...
    fees: [
      { feature: "🪪 簽 / 撤銷意圖", free: "—", paid: "0.1% × 總 USD 上限，一次性，封頂 \\$1" },
  ...
  en: {
    ...
    sections: [
      {
        ...
        steps: [
          ...
          { step: "II", title: "Ticking protocols \"just in case\"", desc: "For convenience, all 8 protocols get checked. Problem: Drift handles derivatives, which is a different risk tier entirely from USDC lending on Kamino. An uninformed check equals consent to the agent opening leverage on Drift. Principle: tick only protocols you have personally used and whose risk profile you know. Skip what you don't recognize — the loss upside is bounded." },
      ...
      },
      {
        ...
        steps: [
          ...
          { step: "II", title: "Tick the minimum protocols + actions needed", desc: "Conservative: Kamino or MarginFi × Lend. Medium: Kamino / MarginFi / Jupiter × Lend + Swap. Aggressive: add Drift × Borrow. Principle is \"tick enough to run the strategy, not one more.\" Every extra protocol or action you tick is another door you hand to the AI." },
    ...
    fees: [
      { feature: "🪪 Sign / revoke intent", free: "—", paid: "0.1% × total USD cap, one-time, capped at \\$1" },
  ...
  ja: {
    ...
    sections: [
      {
        ...
        steps: [
          ...
          { step: "弐", title: "「ついでに」とプロトコルを選びすぎる", desc: "手間を省くために 8 つのプロトコルすべてにチェックを入れる。問題：Drift はデリバティブを扱い、Kamino の USDC 貸付とは、リスク階層そのものが違う。不勉強な選択は、エージェントが Drift でレバレッジを開くことへの同意である。原則：自分で使ったことがあり、リスクプロファイルを知っているプロトコルだけを選ぶ。不慣れなものは外せ——損失の上限が有限のままである。" },
      ...
      },
      {
        ...
        steps: [
          ...
          { step: "弐", title: "戦略に基づき、必要最小限のプロトコル + 動作を選ぶ", desc: "保守：Kamino または MarginFi × Lend。中程度：Kamino / MarginFi / Jupiter × Lend + Swap。攻撃的：さらに Drift × Borrow を加える。原則は「戦略を実行するのに足るだけ選び、余分に 1 つも加えぬ」。余分に選ぶ 1 つは、AI に渡す扉が 1 つ増えるに等しい。" },
    ...
    fees: [
      { feature: "🪪 意図の署名 / 取消", free: "—", paid: "0.1% × 累計 USD 上限、一回限り、上限 \\$1" },
